...
int main()
{
    ...
    int max_demand[5][4] = {{0, 0, 1, 2},  // P0 // MAX_DEMAND Matrix
                            {1, 7, 5, 0},  // P1
                            {2, 3, 5, 6},  // P2
                            {0, 6, 5, 2},  // P3
                            {0, 6, 5, 6}}; // P4

    int available__resources[4] = {1, 5, 2, 0}; // Available Resources
        ...
        for (i = 0; i < n; i++) // Checks for all the instances
        {
            if (finish[i] == 0) // any instance with false sign (which is not in sequence)
            {
                ...
                {                                                       // one process finished successfully as flag remained 0
                    ...
                    finish[i] = 1;                                      // then mark this process as true (won't be checked again)
                }
            }
        }
    }
